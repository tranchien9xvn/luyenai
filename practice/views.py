...
DELETE_PASSWORD = "12345"  # Bạn có thể đưa vào settings nếu muốn

def delete_exercise(request, exercise_id):
    if request.method == 'POST':
        password = request.POST.get('delete_password')
        if password != DELETE_PASSWORD:
            messages.error(request, 'Mật khẩu xóa không đúng!')
            # Trả về lại trang danh sách hoặc chi tiết
            return redirect('practice:practice_home')  # Hoặc redirect về trang hiện tại
        exercise = get_object_or_404(Exercise, id=exercise_id)
        exercise.delete()
        messages.success(request, 'Xóa bài tập thành công.')
        return redirect('practice:practice_home')
    else:
